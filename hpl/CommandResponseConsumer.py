...
from master import hpl_master_list
from master.hpl_master_list import NETWORK
...
class CommandResponseConsumer(threading.Thread):
    ...
    def run(self):
        ...
        response_consumer = KafkaConsumer(bootstrap_servers='pgcil-hpl.probussense.com:9092',
        ...
        for message in response_consumer:
            ...
            network = ""
            if int(node_id) in hpl_master_list.NODE_LIST_20:
                network = NETWORK[0]
            elif int(node_id) in hpl_master_list.NODE_LIST_6F:
                network = NETWORK[1]
            elif int(node_id) in hpl_master_list.NODE_LIST_9A:
                network = NETWORK[2]
            else:
                network = "other"

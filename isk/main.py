...
def sendMeterPassword(gatewayid, sinkid, nodeid):
    ...
    if m_pass is None:
        ...
        print("gateway node_id#" + gatewayid + " sinkid#" + sinkid + " node.py node_id#" + str(nodeid))
...
def on_node_init_rcv(data):
    # print("node.py init rcv for the all_nodes")
    try:
        date_dir = str(date.today())
...
def setmqtt():
    ...
    #
    wni = WirepasNetworkInterface('pgcil-iskraemeco-wnt.probussense.com', 8883, 'mqttmasteruser',
                                  'fBD6L0jOSbhLL2H2AOlZvWcpdSw1gq')
    ...
    wni.register_data_cb(probus_app_data, network=16593952)
    wni.register_data_cb(probus_app_data, network=16594031)
    wni.register_data_cb(probus_app_data, network=16594074)
...
def check_dirs(node_date):
    ...
    if not isExist:
        ...
        print("node.py command response")
